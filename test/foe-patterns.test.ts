...
describe('ближники + лекарь: налётчики и костоправ', () => {
  ...
  it('смоук: перелечка глушит мету добивания — «руби ближайшего» дешевле', () => {
    // выборка шире обычной: с бросками (план damage-types) разрыв двух
    // формулировок здесь — единицы процентов, на 20 сидах он тонет; после
    // MAP (план action-economy) 60 сидов дают уже перевёрнутый знак от шума,
    // на 200 премиса стоит: 0.628 hp у «руби ближайшего» против 0.599
    const chop = sweep(() => partyWith([]), gang, 'late', 200);
    const pick = sweep(
...
describe('танк + кастеры: огр и поджигатель', () => {
  ...
  it('смоук: залпы жгут кучную партию, «держать интервал» бережёт и глушит касты', () => {
    ...
    // ~0.58 вместо прежних ~0.46, премиса «дорогой бой» держится
    expect(hpNaive / 20).toBeLessThan(0.62); // артиллерия за пробкой — дорогой бой
