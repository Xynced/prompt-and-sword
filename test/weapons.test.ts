...
describe('оружие видно игроку', () => {
  it('разведка рядового: имя и урон тесака', () => {
    ...
    // тип урона стоит рядом с уроном (план damage-types): по нему игрок
    // выбирает, чем бить эту броню
    expect(intel.lines).toContain('оружие: ржавый тесак (удар 5, рубящий)');
  });

  it('карточка мастера: три оружия с приёмами через «;»', () => {
    expect(describeWeapons(YAR)).toBe(
      'копьё, колющий (укол копьём 3 (даль 2); отмах древком 5 (даль 2)); ' +
        'меч, рубящий (серия уколов 4); ' +
        'молот, дробящий (пролом 8 (пробивает укрытия); сплеча 12 (открывает))',
    );
  });
});
