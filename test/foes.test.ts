...
describe('поздние враги', () => {
  ...
  it('разведка показывает поведение ПОСЛЕ линзы (фанатик-берсерк без искажений отступления — их нет)', () => {
    ...
    expect(intel.lines).toEqual([
      'вождь орды: ломать самых опасных',
      'вождь орды: крови не жалеть',
      'вождь орды: дыхание орды',
      'вождь орды: бить на упреждение',
      'оружие: обсидиановый топор (удар 9, рубящий) · ритуал 5×5 (замах виден за ход, раз в 3 раунда)',
      // защиты видны в разведке (план damage-types): слово об уязвимости
      // бессмысленно, если игрок не может узнать уязвимость
      'защита: КБ 18 · стойкость 12/реакция 8/воля 10',
      // реакция босса (план reactions) — в разведке: правило «мимо вождя не
      // пробежать» игрок обязан узнать до боя, а не смертью героя
      'реакция (одна в раунд) — ответный удар: кто уходит бегом из-под носа — получает удар',
    ]);
  });
