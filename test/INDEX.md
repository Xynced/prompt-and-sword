...
| [ritual.test.ts](ritual.test.ts) | Ритуал (АОЕ): гейты перезарядки/лимита/висящей зоны, замах → залп через ход тем же центром, ритм ≥3 раундов, смерть кастера отменяет зону |
