...
const CONCEPT_SPECS: Record<ConceptId, string> = {
  ...
  'act.wait':
    '{"id":"act.wait"} — ждать: не идти навстречу врагу и беречь ход, пока бой не докатился; подошедшего врага всё равно бьёт. Замысел «подожди, а ПОТОМ …» — это ДВЕ фразы: выжидание под отрицанием момента ({"condition":{"id":"not","cond":<условие момента>},"preference":{"id":"act.wait"}}) и вторая — с тем же условием без «не» и нужным действием',
  'cond.hpAbove':
  ...
  'cond.weaponFails':
    '{"id":"cond.weaponFails"} — условие: моё оружие не берёт ближайшего врага — он держит или вовсе не чувствует все типы урона моего арсенала (повод сменить цель или отойти)',
  'cond.lull':
  ...
  'sel.vulnerable':
    '"sel.vulnerable" — селектор врага: уязвимый — тот, кого моё оружие берёт лучше прочих (слабость к типу моего урона; нет таких — ближайший)',
  'sel.armored':
    '"sel.armored" — селектор врага: бронированный — с самым высоким КБ; годится и как цель («вали латника»), и как то, от чего держатся подальше',
...
5а. Союзы между УСЛОВИЯМИ — грамматика, отдельных слов не требуют: «если А И Б — …» выражается {"id":"and","conds":[<условие>, <условие>]}, «если А ИЛИ Б — …» — {"id":"or","conds":[…]} (в обоих 2–3 звена из открытого словаря; комбинаторы друг в друга не вкладываются). and горит при всех условиях, or — при любом; or одним правилом предпочтительнее двух фраз с тем же действием.
5б. Отрицание — тоже грамматика: «если НЕ А» выражается {"id":"not","cond":<простое условие>}. Внутрь «не» идёт только ПРОСТОЕ условие — ни «всегда», ни and/or, ни второе «не»; зато само «не» годится как звено внутри and/or. Этим переводятся обороты «пока не …», «до тех пор пока не …», «только когда ещё не …», «если никто не …»: «жди, пока наш не сцепится с врагом» — это {"id":"not","cond":{"id":"cond.allyEngaged"}} с нужным действием, а НЕ безусловная фраза. Отрицание буквально: условие, которое молчит без данных (нет высот на арене, нет таймера у сценария), под «не» истинно.
