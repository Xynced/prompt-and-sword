...
| [vocab.ts](vocab.ts) | Словарь концептов (35: условия, селекторы, действия, манера удара, пространство), стартовый/базовый/глубокий наборы | `ConceptId`, `CONCEPTS`, `STARTING_VOCAB`, `UNLOCKABLE` |
...
| [scoring.ts](scoring.ts) | Utility-скоринг: выбор одного действия на очко хода, инстинкты + веса правил, высота и каменное укрытие, топ-3 фактора решения | `generateCandidates`, `decide`, `ActionKind`, `AP_COST`, `Decision`, `Factor`, `makeCtx`, `rangeAt` |
