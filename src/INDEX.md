...
|---|---|---|
| [types.ts](types.ts) | Базовые типы: сторона, позиция, юнит, виды действий, оружие (урон/дальность на нём), площадные формы, id линз | `Side`, `Pos`, `ActionKind`, `LensId`, `CombatUnit`, `WeaponSpec`, `AoeSpec` |
...
| [cards.ts](cards.ts) | Обратный перевод IR после линзы → карточка «как понял» (искажения ⚠); описания оружия для разведки и карточек | `understandingCard`, `describeWeapon`, `describeWeapons` |
