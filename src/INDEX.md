...
| [ir.ts](ir.ts) | IR: правило «условие → предпочтение → вес», структурные пометки линз, условия-триггеры (hpAbove, firstBlood, leaderDown, wasHit), комбинаторы условий `and`/`or` (глубокие чипсы), предпочтения внимания (taunt, lure), вычисление условий и селекторов | `Rule`, `LensMark`, `Condition`, `Selector`, `Preference`, `evalCondition`, `resolveSelector` |
| [vocab.ts](vocab.ts) | Словарь концептов (71: условия, селекторы, действия, манера удара, темп, пространство, активы; вторая партия слов — условия под чипсы «и», контр-селекторы, добивание/фокус-огонь, гейты bless/feint; внимание — «вызывать на себя»/«уводить от X»); старт + обычные/редкие по данным аудита слов; изъятые из обращения | `ConceptId`, `CONCEPTS`, `STARTING_VOCAB`, `COMMON_WORDS`, `RARE_WORDS`, `RETIRED_WORDS`, `UNLOCKABLE` |
...
| Модуль | Что делает | Ключевые экспорты |
|---|---|---|
| [constructor.ts](constructor.ts) | Конструктор фраз: `PhraseDraft` (чипсы словаря) → IR без LLM; условия «и»/«или» (комбинаторы), вложенные группы `if (a) { if (b) … }` → плоские правила; закрытый концепт — ошибка | `compilePhrase`, `compileNested`, `PhraseDraft`, `PhraseGroupDraft`, `CompileResult` |
...
| [ui/main.ts](ui/main.ts) | Весь веб-UI одним файлом (чистый DOM + Vite): карта, бой, конструктор (вложенные чипсы условий до трёх уровней со связкой «и»/«или»), спарринг, журнал; анимации действий в бою — всплывающие цифры урона/лечения, вспышки задетых клеток АОЕ (залп/линия/ритуал), значки прикрытий на фишках (прикрытие/глухая оборона/щит союзника); панель задачи боя на узле и в бою, фикс-расстановка сценария, кнопка «пропустить урок»; тест-кнопки «debug» и «свободный режим» (весь словарь разом); панель отладки «собрать бой» — сценарий × партия × характеры |
