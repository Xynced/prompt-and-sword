...
/**
 * Щит (план armor) — снаряжение танка, работающее двумя каналами pf2e:
 * поднятый щит прибавляет `ac` к КБ (бонус обстоятельств, как укрытие), а
 * прошедший удар гасится на `hardness`. Погашенное копится: набрав `hp`
 * вмятин, щит ломается и до конца боя не даёт ни бонуса, ни блока.
 *
 * Числа своей шкалы: удар в этом симе — 2–7 hp, поэтому pf2e-твёрдость 5 и
 * BT 10 были бы выключателем; рабочий диапазон — твёрдость 2–4, запас 8–14.
 */
export interface ShieldSpec {
  /** Бонус к КБ, пока щит поднят. */
  ac: number;
  /** Сколько урона гасит блок — раз в раунд (в pf2e это реакция). */
  hardness: number;
  /** Запас щита: набрав столько погашенного урона, щит ломается. */
  hp: number;
}
...
export type ActionKind =
  ...
  | 'cover'
  | 'fullCover'
  /** Поднять щит (план armor): 1 AP, бонус щита к КБ и право на блок до своего следующего хода. */
  | 'raiseShield'
  | 'shieldAlly'
...
export interface ActiveSpec {
  ...
  /**
   * Стена (щитоносец): 2 AP — прикрытие COVER_AC себе и всем смежным союзникам
...
export interface PassiveSpec {
  /** Щитоносец (Гром): его прикрытие союзника даёт ac к КБ вместо общего COVER_AC. */
  shieldwall?: { ac: number };
  ...
  shadow?: { mult: number };
  /** В спину (Тесса, волк, душегуб): фланг застигает цель глубже общего OFF_GUARD_AC. */
  sneak?: { offGuard: number };
  /** Кара (Заря): атаки ×mult по врагу, чей удар последним получил кто-то из своих. */
  retribution?: { mult: number };
  /**
   * Град стрел (Дарт, hunter's edge flurry pf2e): лестница MAP мягче на две
   * ступени — волюм-стрелок платит за третий выстрел меньше прочих.
   */
  flurry?: true;
...
export interface WeaponMove {
  ...
  sure?: true;
  /** Цели остаётся эта доля бонуса обстоятельств к КБ (канал HARD_PIERCE); влечёт sure. */
  pierce?: number;
  ...
  twin?: true;
  /**
   * Парный приём (план action-economy, волна 6): два удара по одной цели за
   * одно действие. Оба броска идут по текущему MAP — штраф растёт только
   * после приёма (pf2e: Flurry of Blows, Twin Feint). Множитель задаётся
   * **на удар**: приём стоит вдвое.
   */
  pair?: true;
...
export interface WeaponSpec {
  ...
  persist?: PersistSpec;
  /**
   * Ловкое оружие (agile pf2e: кинжалы, кулаки, парные ножи) — ступень MAP
   * мягче (`MAP_STEP_AGILE`): второй и третий удары за ход обходятся дешевле.
   */
  agile?: true;
...
/**
 * Реакция юнита (план reactions) — что он делает в **чужой** ход. Валюта
 * одна на раунд (`reactionUsed`), поэтому реакция — это всегда выбор, а не
 * прибавка. Живёт своим полем, а не в `PassiveSpec`: пассив всегда включён,
 * реакция тратится.
 *
 * Волна 2 — только `reactiveStrike`; остальные семейства (защита от удара по
 * себе, вступиться за своего, погоня за уходящим) идут волнами 4–5.
 */
export type ReactionKind =
  /** Ответный удар (Reactive Strike, воин): смежный уходит бегом — получает удар. */
  | 'reactiveStrike'
  /** Не уйдёшь (No Escape, варвар): уходящий не отрывается — шаг следом, контакт держится. */
  | 'noEscape'
  /** Сорвать добычу (Disrupt Prey, следопыт): помеченный уходит бегом в дальности — выстрел вслед. */
  | 'disruptPrey'
  /** Воздаяние (Retributive Strike, паладин): ударили смежного своего — удар по обидчику. */
  | 'retributiveStrike'
  /** Заступление (жрец): своего в дальности ранили — малое лечение сразу. */
  | 'succor'
  /** Уворот (Nimble Dodge, плутовка): удар по мне идёт против бонуса к КБ. */
  | 'nimbleDodge'
  /** Отбить стрелу (Deflect Arrow, монах): то же, но только против стрелкового — и вдвое крепче. */
  | 'deflectArrow'
  /** Щит (Shield + Shield Block, волшебница): бонус к КБ и поглощение; сработав, гаснет до конца боя. */
  | 'arcaneShield';
...
export interface CombatUnit {
  ...
  alive: boolean;
  /**
   * Бонус обстоятельств к КБ от своего действия (план armor): 0, COVER_AC,
   * BRACE_AC. Держится до своего следующего хода. Бонусы обстоятельств не
   * складываются (правило pf2e) — по всем каналам берётся высший.
   */
  guard: number;
  /**
   * Чем поставлен бонус: величина у глухой обороны общая с укрытием, а рипост,
   * цена дороги и повышение укрытия (Take Cover из pf2e) должны узнавать само
   * действие. Снимается вместе с guard.
   */
  guardFrom?: ActionKind;
  /**
   ...
   * хода вместе с guard.
   */
  guardedBy?: { id: string; bonus: number };
  /** Отчаянный удар открыл: входящий урон ×SELFLESS_VULN_MULT до своего следующего хода. */
  exposed: boolean;
  /**
   * Ударов сделано за текущий ход (план action-economy, волна 6) — по нему
   * считается MAP. Сбрасывается в начале своего хода; парный и сдвоенный
   * приёмы прибавляют сразу два, но оба своих броска делают по прежнему
   * значению.
   */
  strikes?: number;
  /**
   ...
   * слабый удар крепче, «наверняка» — полный удар режет бонус обороны,
   * «приманка» — плавающий бонус к КБ BAIT_AC, «вызывать на себя» (план
  ...
  /** Щит (план armor): его носят танки — «поднять щит» и блок по твёрдости. */
  shield?: ShieldSpec;
  /** Погашено щитом за бой; дойдя до `shield.hp`, щит ломается. */
  shieldDents?: number;
  /** Щит сломан: ни бонуса, ни блока до конца боя. */
  shieldBroken?: boolean;
  /** Что юнит умеет делать в чужой ход (план reactions); у большинства нет. */
  reaction?: ReactionKind;
  /** Щит волшебницы уже сработал — до конца боя (одноразовая реакция). */
  arcaneShieldSpent?: boolean;
  /**
   * Реакция потрачена — до своего следующего хода (план reactions): одна
   * валюта на всё, что боец делает в чужой ход. Блок щита, перехват
   * телохранителя и рипост глухой обороны берут её из общего кармана, как в
   * pf2e, поэтому танк больше не реагирует дважды за раунд.
   */
